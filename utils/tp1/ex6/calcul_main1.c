...
void calcul1(void);
void calcul1_reset(void);
void calcul1_I_OPERANDE(int i);
void calcul1_I_ADDITION(void);
void calcul1_I_SOUSTRACTION(void);
void calcul1_I_DIVISION(void);
void calcul1_I_MULTIPLICATION(void);

/* Fonction pour le signal de sortie, a ecrire par le programmeur */
void calcul1_O_LCD(int i) {
	printf("LCD = %d\n",i);
}
...
int main(int argc, char* argv[]) {
    ...
    while(1) {
        sleep(10);
        printf("Saisir un operande ou un operateur (+/-*) :");
        ...
        if(sscanf(s,"%d",&value)==1)
            calcul1_I_OPERANDE(value);
        else if(strchr(s,'+')!=NULL)
            calcul1_I_ADDITION();
        else if(strchr(s,'-')!=NULL)
            calcul1_I_SOUSTRACTION();
        else if(strchr(s,'*')!=NULL)
            calcul1_I_MULTIPLICATION();
        else if(strchr(s,'/')!=NULL)
            calcul1_I_DIVISION();
